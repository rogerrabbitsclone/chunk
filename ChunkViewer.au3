...
; Read settings file.
$RepeaterAddress = 		IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "Repeater", "Address", "" )
$RepeaterAddressLAN = 	IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "Repeater", "AddressLAN", "" )
$RepeaterViewerPort =	IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "Repeater", "ViewerPort", "" )
$IDList = 				IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "List", "" )
$IDListMax = 			IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "ListMax", "" )
$Quality = 				IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "Quality", "" )
$LANMode = 				IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "LANMode", "" )

; AutoScale connect string
$AutoScale =			IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "AutoScale", "0" )
If $AutoScale = 1 Then
...
$Password =				IniRead( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "Password", "" )
If $Password <> "" Then
...
; Create the GUI.
$Form1 = GUICreate( "ChunkVNC Viewer", 300, 80, -1, -1 )
	...
	Switch $nMsg
		...
		Case $ContextMenuMode1
			...
			If $LANMode = 0 Then
				...
				IniWrite( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "LANMode", "1" )

			Else

				IniWrite( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "LANMode", "0" )
		...
		Case $ContextMenuHistory1
			...
			IniWrite( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "List", "" )


		; About
		Case $ContextMenuAbout1

			GUICtrlSetData( $Label1, "ChunkVNC - a creation by supercoe." )
		...
		Case $Button1
			...
			IniWrite( @ScriptDir & "\Bin\chunkviewer.ini", "ChunkViewer", "List", $IDList )
